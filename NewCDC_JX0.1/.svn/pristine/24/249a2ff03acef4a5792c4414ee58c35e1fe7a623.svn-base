...
public class DragListView extends ListView implements OnScrollListener,
	...
	public void initDragListView(Context context) {
		...
		initHeadView(context, mLastUpdateTime);// 初始化该head.
	 ...
	 *            上次更新时间
	 */
	public void initHeadView(Context context, String lastRefreshTime) {
		...
		mLastUpdateTime = SharePreferenceUtilDaoFactory.getInstance(context)
				.getCollectionTime();
		// 显示更新事件
		if (mLastUpdateTime != null && !"".equals(mLastUpdateTime)) {
			mLastUpdateTextView.setText("最近更新:" + mLastUpdateTime);
		}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		// 显示更新事件
		if (mLastUpdateTime != null && !"".equals(mLastUpdateTime)
				&& !tempUpdateTime.endsWith(mLastUpdateTime)) {
			tempUpdateTime = mLastUpdateTime;
			Log.e("mLastUpdateTime", mLastUpdateTime);
			mLastUpdateTextView.setText("最近更新:" + mLastUpdateTime);
		}
		switch (ev.getAction()) {
		// 按下
	...
	private void switchViewState(DListViewState state) {
		...
		case LV_NORMAL: {
			...
			mArrowImageView.setImageResource(R.drawable.progress);
		}
			break;
		// 下拉状态
		case LV_PULL_REFRESH: {
	...
	public void setmFirstItemIndex(int firstItemIndex) {
